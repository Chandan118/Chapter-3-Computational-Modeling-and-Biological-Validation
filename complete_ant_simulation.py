...
class CompleteAntSimulation:
    ...
    def test_netlogo_connection(self) -> bool:
        ...
        try:
            ...
            self.netlogo = netlogo_utils.init_netlogo(
                gui=False, 
        ...
        try:
            ...
            for tick in range(0, self.trial_duration, self.measure_interval):
                ...
                if tick % 200 == 0:
                    ...
                    print(f"  Progress: {progress:5.1f}% | Tick: {tick:4d} | "
    ...
    def _calculate_metrics(self, data: Dict) -> Dict:
        ...
        return {
            ...
            'discovery_time': next((i for i, v in enumerate(data['food_collected']) if v > 0), -1) * self.measure_interval
        }
    ...
    def _save_trial_data(self, data: Dict):
        ...
        for key, value in data.items():
            if isinstance(value, (list, np.ndarray)):
                ...
                save_data[key] = {k: (float(v) if isinstance(v, (np.integer, np.floating)) else v) 
    ...
    def _create_visualizations(self, df: pd.DataFrame, results: List[Dict]):
        ...
        try:
            ...
            fig.suptitle('Ant Colony Simulation - Performance Metrics', 
            ...
            fig.suptitle('Time Series Analysis - First Trial Each Colony', 
                        fontsize=16, fontweight='bold')
            
            # Plot first trial of each colony
            for colony_id in range(1, min(self.num_colonies + 1, 6)):
                trial_data = next((r for r in results 
                                  if r['colony_id'] == colony_id and r['trial_id'] == 1), None)
                if trial_data:
                    label = f'Colony {colony_id}'
                    
                    # Food collected
                    axes[0, 0].plot(trial_data['ticks'], trial_data['food_collected'], 
                                   label=label, linewidth=2)
                    
                    # Foraging ants
                    axes[0, 1].plot(trial_data['ticks'], trial_data['ants_with_food'], 
                                   label=label, linewidth=2)
                    
                    # Pheromone level
                    axes[1, 0].plot(trial_data['ticks'], trial_data['pheromone_level'], 
                                   label=label, linewidth=2)
                    
                    # Total ants
                    axes[1, 1].plot(trial_data['ticks'], trial_data['ants_total'], 
                                   label=label, linewidth=2)
